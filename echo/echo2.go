...
func main() {
	s, sep := "", ""
	/*
		In each iteration of the loop. range produces a pair of values: the index and the value of element at that index.
		Here we don't need index values, so a basic idea is to use a temporary variable lie temp and ignore its value,
		but Go does not permit unused local variales, so this would result in a compilation error.
